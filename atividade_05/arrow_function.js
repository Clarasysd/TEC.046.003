...
const metros = [1, 5, 10];
const centimentros = metros.map(m => m * 100);
centimentros.forEach (c => console.log (`${c}cm`));

// .filter(): Cria uma lista com os itens especificados
const notas = [8, 4, 9, 5];
const notas_baixas = notas.filter = notas.filter (n => n < 6);
...
produtos.filter (p => p.includes ("Dell")).forEach (i => console.log (i));
